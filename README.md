...
* **多维时间跨度**：支持一键切换 `最近3天`（今天向前覆盖2天）、`最近7天`（今天向前覆盖6天）、`本月`。数据无缝重绘，无需刷新页面。
* **深度缓存命中分析**：不仅在卡片顶部提供当前时间跨度的综合命中率，更在图表 `Tooltip` 悬浮框内动态注入单日的精确缓存命中率计算。
* **个性化价格预测弹窗**：新增独立的价格预测面板，可输入预计总 token 和多个命中率档位，直接预测 `v4-flash` 与 `v4-pro` 的输入成本、输出成本与预计总花费。
* **模型级比例预测与降级策略**：价格预测优先使用当前时间范围内的模型专属输出/输入比例；若数据不足，会依次回退到全量历史模型比例、当前整体比例、全量历史整体比例，最后才使用默认比例，并在弹窗中明确提示比例来源。
* **千分位输入与调试日志**：预计输入 token 支持自动千分位分隔，提升录入可读性；点击生成价格表时，还会在控制台输出参考跨度、样本输入输出、模型比例来源和预测明细，方便核对。
...
个性化价格预测弹窗会基于官方每 `1M tokens` 价格与本地捕获的历史用量比例，分别预测 Flash / Pro 两个模型的成本，并展示当前采用的比例来源。

![个性化价格预测弹窗](assets/predict.png)
...
* **价格预测**: 基于 `/api/v0/usage/amount` 中的 `PROMPT_CACHE_HIT_TOKEN`、`PROMPT_CACHE_MISS_TOKEN` 与 `RESPONSE_TOKEN` 统计各模型输出/输入比例，并按官方 `每 1M tokens` 单价做编码预测。
